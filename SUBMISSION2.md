...
| Advanced Features | JSON persistence, order history, wallet payments, customer payment history, checkout receipts, admin reporting, product reviews, low-stock detection and alerts, advanced product filtering, and back navigation support realistic backend behavior beyond the first core flow. |
...
| Testing / Validation | `dotnet build` verifies compilation; `Tests/OnlineShoppingSystem.Tests.csproj` verifies domain and service behavior; runtime validation is handled by domain/service guard clauses and menu-level exception handling. |
...
- Checkout prints a receipt with order, item, payment, and remaining wallet details.
- Customers can view payment history linked to their orders.
- Inventory control reduces stock during checkout and allows administrator restocking.
- Administrators see a low-stock alert when products are at or below reorder level.
...
- Product filtering supports keyword, category, price range, minimum rating, and in-stock views.
...
- Cart total calculation and quantity updates.
- Product filtering with LINQ-backed service methods.
- Duplicate username validation.
- Wallet payment failure when balance is too low.
- Successful checkout creating an order, payment, and stock reduction.
